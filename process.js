...
// api/process.js - Vercel Serverless Function (Multi-Bot Isolated Verification + Failed/VPN Audit Logging)
export default async function handler(req, res) {
  ...
  try {
    ...
    // Cloudflare IP Country & Threat Headers
    ...
    // =========================================================================
    // HELPER: AUDIT LOGGING HELPER (Saves all Attempts/Failures to Database)
    // =========================================================================
    const logAuditRecord = async (statusLabel, reasonMsg) => {
      try {
        ...
        console.warn('Audit insert warning:', err.message);
      }
    };

    // =========================================================================
    // CHECK 0: VPN / PROXY / TOR DETECTION
    // =========================================================================
    const isVpnHeader = cfCountry === 'T1' || cfCountry === 'XX';

    if (isVpnHeader) {
      await logAuditRecord('VPN_BLOCKED', `VPN Detected (CF Country: ${cfCountry || 'PROXY'})`);
    ...
    // =========================================================================
    // CHECK 0.1: MANDATORY VALIDATIONS
    // =========================================================================
    if (!user_id || !device_id) {
      await logAuditRecord('FAILED', 'Missing User ID or Device ID');
      return res.status(200).json({
        status: 'failed',
    ...
    if (cloneCheck && cloneCheck.length > 0 && String(cloneCheck[0].user_id) !== String(user_id)) {
      await logAuditRecord('CLONE_ATTEMPT', 'Same device used by another Telegram ID');
